...
const ChatWindow = ({ doctor, onDoctorChange }) => {
  ...
  return (
    <div className="chat-window bg-white p-4 rounded-lg shadow-lg max-w-3xl mx-auto flex flex-col h-full">
      {/* Header */}
      <div className="chat-header flex items-center border-b pb-3 mb-4">
        <div className="relative w-12 h-12 mr-4">
          <img
      ...
      {/* Message List */}
      <div className="message-list overflow-y-auto flex-grow mb-4 max-h-80">
        {messages.map((msg, index) => (
          <div
            key={index}
            className={`message ${msg.sender === "patient" ? "text-right" : "text-left"} mb-3`}
          >
            {msg.type === "text" && (
              <div
                className={`message-text ${msg.sender === "patient" ? "bg-blue-500 text-white" : "bg-gray-200 text-black"} p-2 rounded-lg inline-block`}
              >
                {msg.content}
              </div>
            )}
      ...
      {/* Video Call Modal */}
      {isVideoCall && (
        <div className="video-call-modal fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="video-call-window bg-white p-6 rounded-lg shadow-lg w-full max-w-4xl flex flex-col">
            <div className="video-call-header flex justify-between mb-4">
              <h3 className="text-xl font-semibold">Video Call</h3>
              <button
                onClick={endVideoCall}
            ...
            <div className="video-call-body grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="video-call-local">
                <video
                  ref={videoRef}
                  autoPlay
