...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Fetch user data from Firestore to determine account type
      const userDocRef = doc(db, "users", user.uid);
      const userDoc = await getDoc(userDocRef);
      
      if (userDoc.exists()) {
        const userData = userDoc.data();
        const accountType = userData.accountType;  // Get account type from Firestore

        // Redirect to the corresponding dashboard based on the account type
        ...
        } else if (accountType === 'labtech') {
          navigate("/labtech-dashboard");  // Redirect to lab technician dashboard
        }
      }
